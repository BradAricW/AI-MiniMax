...
//other declarations
char b[8][6];
int compKing, playKing, compPiece, playPiece, compSpecial, playSpecial;
int maxDepth = 6;
...
void checkGameOver(int m)
{
	...
	if (checkWin(0) == -50000)
	{
		std::cout << std:: endl <<  "Congratulations! You win!" << std::endl;
		exit(0);
	}
	else if (checkWin(0) == 50000)
	{
		std::cout << std:: endl <<  "Computer wins! Better luck next time." << std::endl;
		exit(0);
	}
	...
	if(m == 1)
	{
		...
		std::cout << std::endl << "Computer has " << moveCount << " possible moves" << std::endl;
		if (test == 0)
		{
			std::cout << std:: endl <<  "Congratulations! You win!" << std::endl;
			exit(0);
				...
				for (int m = 0; m < 6; m++)
				{
					for (int n = 0; n < 8; n++)
					{
						...
						if (test == 1)
						{
							...
							if (holdContentsDest == 'b' || holdContentsDest == 'h')
							{
								playSpecial--;
								specialCount++;
							}
